...
uint256 hashGenesisBlock("0x899827c218c2dcb767ade5546f1544657696178426a285e3fe6d2b3de239e4dc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 500Coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "500Coin Signed Message:\n";
...
{
    // 500Coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // 500Coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // 500Coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // 500Coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x899827c218c2dcb767ade5546f1544657696178426a285e3fe6d2b3de239e4dc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1398512259;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084928263;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("500CoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("500CoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running 500Coin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("500CoinMiner terminated\n");
        throw;
    }
}
